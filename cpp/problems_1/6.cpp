...
ListNode *mergeTwoLists(ListNode *list1, ListNode *list2) {
  ...
  Output: []
  */
  auto list_one = listToVector(list1);
  auto list_two = listToVector(list2);
  std::vector<int> my_vec;

  std::merge(list_one.begin(), list_one.end(), list_two.begin(), list_two.end(),
             std::back_inserter(my_vec));

  for (auto e : my_vec) {
    std::cout << e << std::endl;
  }
  return createList(my_vec);
}
int main() {
